// @ts-nocheck
// Vendored from: https://gist.github.com/sebastien-lempens/f9318c430500e4ac9b7160a0322db4d6
// MIT — Sebastien Lempens, r3f-flow-field-particles
// Prototype integration for the particle-orb branch. Type-checking disabled
// intentionally; revisit if we keep this scene long-term.
  ...
  forwardRef((props, ref) => {
    const uniforms = useMemo(() => {
      return {
        ...
        // Must seed with 2 Color instances — shader declares vec3 uColors[2].
        // An empty array crashes three.js with `undefined.toArray()` on first
        // upload before useEffect populates real values.
        uColors: new Uniform([new Color(), new Color()]),
...
const InitMeshWrapper = forwardRef(
  ({ children, visible, onUpdate, onPointerMove }, ref) => {
...
const FlowFieldParticles = ({
  ...
  const gpgpu = useMemo(() => {
    ...
    for (let i = 0; i < modelGeometry.count; i++) {
      dataTexture.image.data[i * 4 + 0] =
        modelGeometry.attributes.position.array[i * 3 + 0];
      dataTexture.image.data[i * 4 + 1] =
        modelGeometry.attributes.position.array[i * 3 + 1];
      dataTexture.image.data[i * 4 + 2] =
        modelGeometry.attributes.position.array[i * 3 + 2];
      dataTexture.image.data[i * 4 + 3] = Math.random();
    }

    const particlesVariable = GCR.addVariable(
      "uParticles",
      GpgpuFragmentShader,
      dataTexture
    );
    ...
    particlesVariable.material.uniforms.uBaseParticlesTexture = new Uniform(
      dataTexture
    );
    particlesVariable.material.uniforms.uDisturbIntensity = new Uniform(
      disturbIntensity
    );
    particlesVariable.material.uniforms.uRepulsionForce = new Uniform(
      repulsionForce
    );
    particlesVariable.material.uniforms.uMouse = new Uniform(
      new Vector3(0, 0, 0)
    );
  ...
  const particles = useMemo(() => {
    ...
    geometry.setAttribute(
      "aParticlesSize",
      new BufferAttribute(particlesSizeArray, 1)
    );

    modelGeometry.attributes.color &&
      geometry.setAttribute(
        "aParticlesColor",
        new BufferAttribute(modelGeometry.attributes.color.array, 3)
      );

    modelGeometry.attributes.position &&
      geometry.setAttribute(
        "position",
        new BufferAttribute(modelGeometry.attributes.position.array, 3)
      );

    modelGeometry.attributes.uv &&
      geometry.setAttribute(
        "aMeshUv",
        new BufferAttribute(modelGeometry.attributes.uv.array, 2)
      );
  ...
  useEffect(() => {
    ...
    if (particlesRef.current) {
      particlesRef.current.geometry.setAttribute(
        "aNormal",
        modelGeometry.attributes.normal
      );
    }
    ...
    if (particlesMaterialRef.current) {
      ...
      if (lightSource) {
        ...
        if ("position" in light) {
          ...
          particlesMaterialRef.current.uniforms.uLightSource.value.copy(
            light.position
          );
        }

        if ("color" in light) {
          particlesMaterialRef.current.uniforms.uLightSourceColor = new Uniform(
            light.color
          );
        }

        if ("intensity" in light) {
          particlesMaterialRef.current.uniforms.uLightSourceIntensity =
            new Uniform(light.intensity);
        }
      } else {
        particlesMaterialRef.current.uniforms.uHasLightSource.value = false;
      }

      particlesMaterialRef.current.uniforms.uShape.value =
        ParticleShapeIntValue(shape);

      if (modelMesh?.material?.map) {
        particlesMaterialRef.current.uniforms.uHasColors.value = false;
        particlesMaterialRef.current.uniforms.uMeshMap.value =
          modelMesh.material.map;
      }
    }
  ...
  useFrame(({ clock }) => {
    ...
    mouseDeltaValue = MathUtils.lerp(
      mouseDeltaValue,
      Math.abs(lastMousePosX - mouseRef.current.x),
      0.1
    );

    if (particlesRef.current && modelMesh) {
      particlesRef.current.position.copy(modelMesh.position);
    }

    if (particlesMaterialRef.current && gpgpu) {
      gpgpu.ref.compute();

      gpgpu.particlesVariable.material.uniforms.uTime.value = elapsedTime;
      gpgpu.particlesVariable.material.uniforms.uDeltaTime.value = deltaTime;
      gpgpu.particlesVariable.material.uniforms.uDisturbIntensity.value =
        disturbIntensity;
      gpgpu.particlesVariable.material.uniforms.uRepulsionForce.value =
        repulsionForce;
      gpgpu.particlesVariable.material.uniforms.uMouse.value.copy(
        mouseRef.current
      );
      gpgpu.particlesVariable.material.uniforms.uMouseDelta.value =
        mouseDeltaValue;

      particlesMaterialRef.current.uniforms.uTime.value = elapsedTime;
      particlesMaterialRef.current.uniforms.uResolution.value = [
        gpgpu.size,
        gpgpu.size,
      ];
      particlesMaterialRef.current.uniforms.uParticlesTexture.value =
        gpgpu.ref.getCurrentRenderTarget(gpgpu.particlesVariable).texture;

      lightSource &&
        particlesMaterialRef.current.uniforms.uLightSource.value.copy(
          lightSource.current.position
        );

      helperRef.current?.position
        .copy(mouseRef.current)
        .add(modelMesh.position);
    }
  ...
  return (
    <group ref={ref}>
      ...
      <InitMeshWrapper
        ...
        onUpdate={(e) => {
          // Defer out of R3F's commit phase to avoid
          // "state update on unmounted component" warning.
          if (e) queueMicrotask(() => setInitMeshRef(e));
        }}
        onPointerMove={handlePointerMove}
      >
        {children}
